...
if (($_FILES["image"]["error"] == 4) && ($getAllRow["subcategory_id"] != $subcategory)) {
    ...
    if (is_file($current) && is_writable($current)) {
        // 儲存編輯資料
        $sql = "UPDATE instrument SET name='$name', price=$price, stock=$stock, subcategory_id=$subcategory, info='$info', onshelf_time='$onshelf_time', updated_time='$now' WHERE id=$id";
    ...
    if (is_uploaded_file($_FILES['image']['tmp_name'])) {
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFilePath)) {
            ...
            if (file_exists($current)) {
                unlink($current);
                $sql = "UPDATE instrument SET name='$name', price=$price, stock=$stock, subcategory_id=$subcategory, img='$image', info='$info', onshelf_time='$onshelf_time', updated_time='$now' WHERE id=$id";
                if ($conn->query($sql) === TRUE) {
                    header("location: instrument-detail.php?id=$id");
                } else {
